# PhotoPainter Web UI + HTTP APIs
This directory is served from the SD card as `/system/` and contains the browser UI.
The firmware serves these files from `/sdcard/system`.
...
Behavior
- Files are served from `/sdcard/system`.
- URI mapping:
  - `GET /` -> `/sdcard/system/index.html`
  - `GET /app.js` -> `/sdcard/system/app.js`
  - `GET /dir/` -> `/sdcard/system/dir/index.html`
- Query strings are ignored when mapping to the file path.
...
Safety
- Requests containing `..` or `\` are rejected.
...
## UI integration
The UI (`index.html` + `app.js`) uses:
- `/api/rotation` to set/get rotation
- `/dataUP` to upload the BMP
- `/api/photos*` to list/select/delete/cycle stored photos
